...
// TestParseAuthorizedKeyRest_PinsTheMultiKeyCheck pins the premise the
// multi-key check in identitySigner rests on: x/crypto hands back everything
// after the key it parsed, blank and `#` comment lines included, and only
// skips those on the *next* parse. So "rest is non-blank" and "another key
// follows" are different questions, and an ordinary .pub answers yes to the
// first and no to the second. The check asks the second.
func TestParseAuthorizedKeyRest_PinsTheMultiKeyCheck(t *testing.T) {
	for _, tc := range []struct {
		name        string
		in          string
		restIsEmpty bool
		...
		{
			name:        "one key and a trailing newline",
			in:          ed25519PublicKey + "\n",
			restIsEmpty: true,
		},
		{
			name:        "a blank line and a # comment after the key",
		...
		t.Run(tc.name, func(t *testing.T) {
			_, _, _, rest, err := ssh.ParseAuthorizedKey([]byte(tc.in))
			require.NoError(t, err)
			require.Equal(t, tc.restIsEmpty, len(rest) == 0, "rest = %q", rest)

			_, _, _, _, err = ssh.ParseAuthorizedKey(rest)
			if tc.anotherKey {
				require.NoError(t, err, "a second key parses out of what follows")
			} else {
...
func TestIdentitySigners_MultiKeyFileIsAnError(t *testing.T) {
	file := filepath.Join(t.TempDir(), "authorized_keys")
	require.NoError(t, os.WriteFile(file, []byte(ed25519PublicKey+"\n"+rsaPublicKey+"\n"), 0600))
	_, _, err := identitySigners([]string{file}, "", failingPrompt(t))
	require.EqualError(t, err, file+" names more than one key; name a file with a single key")
}

// TestIdentitySigners_PubWithBlanksAndCommentsIsOneKey is what the multi-key
// check must not catch: the trailing newline, blank line and comment that
// ssh-keygen and editors leave around a single key. With no agent it gets as
// far as the lookup and no further, which is exactly as far as it should.
func TestIdentitySigners_PubWithBlanksAndCommentsIsOneKey(t *testing.T) {
	pubFile := filepath.Join(t.TempDir(), "id.pub")
	require.NoError(t, os.WriteFile(pubFile, []byte(ed25519PublicKey+"\n\n# generated by ssh-keygen\n"), 0600))
	_, _, err := identitySigners([]string{pubFile}, "", failingPrompt(t))
	require.ErrorContains(t, err, pubFile+": no SSH agent to look up", "the key was accepted; only the agent was missing")
	require.NotContains(t, err.Error(), "more than one key")
}
