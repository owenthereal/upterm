...
func setupSignalHandler(g *run.Group, ctx context.Context, shutdownRequested *atomic.Bool) {
...

	{
		...
		// and the command killed during teardown recorded as signaled. The
		// parent context is asked once more before returning, without
		// blocking, because it is the only thing that counts.
