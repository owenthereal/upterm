...
The variable is `build_exit_code` rather than `status`, which zsh reserves. Interrupting `upterm session wait` on its own leaves the session running; in this script it falls through to `session stop`, which ends it. Each `session set` restarts the window from now, so running it again gives more time; `--join-timeout 0` turns it off. A guest who joins at any point, even briefly or only over SFTP, claims the session: the join timeout is off for the rest of its life, and a later `session set` says so. `session info` shows the timeout and its deadline (`joinTimeout`, `joinDeadline` in the JSON). `session info`, `session stop` and `session set` exit 4 when no session has the name.
