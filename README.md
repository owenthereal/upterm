...
The `status` field is `starting`, `ready`, `disconnected` or `ended`, and `reason` is `exited` (with `exitCode`), `signaled`, `stopped`, `startup_failed` or `unknown`. The hosted command sees its own name in `UPTERM_SESSION_NAME`.
